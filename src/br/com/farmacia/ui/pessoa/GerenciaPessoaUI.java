	...
	private void atualizarTabela() {
		...
		for (Pessoa pessoa : pessoas ) {
			modelo.addRow(new Object[]{
				traduzirCpfMostrar(pessoa.getCpf()), 
				pessoa.getNome(), 
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Remover": // Tenho que verificar as restrições antes de remover
			if (linha > -1) {
				...
				cpf = traduzirCnpjProBanco(cpf);
				pessoa = repo.recuperarPorCPF(cpf);
				int opcao = JOptionPane.showConfirmDialog(null, "Isso removerá todos os registros dessa pessoa. Deseja realmente fazer isso?");
